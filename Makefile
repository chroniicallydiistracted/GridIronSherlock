...
generate:
	@bash -lc '$(LOAD_ENV) set -euo pipefail; if [ ! -f "$(OPENAPI)" ]; then echo "$(OPENAPI) not found"; exit 1; fi; mkdir -p packages/sdk; rm -rf packages/sdk/typescript packages/sdk/python; npx --yes @openapitools/openapi-generator-cli@2.25.0 generate -i $(OPENAPI) -g typescript-fetch -o packages/sdk/typescript --skip-validate-spec; npx --yes @openapitools/openapi-generator-cli@2.25.0 generate -i $(OPENAPI) -g python -o packages/sdk/python --skip-validate-spec; echo "SDKs generated under packages/sdk/"'
...
lint: generate
	@bash -lc '$(LOAD_ENV) if [ -d apps/web ]; then cd apps/web && if npm run lint --silent >/dev/null 2>&1; then npm run lint || true; elif command -v npx >/dev/null 2>&1 && ls .eslintrc* >/dev/null 2>&1; then npx eslint . || true; fi; fi; if [ -d services/api ]; then cd services/api && if command -v ruff >/dev/null 2>&1; then ruff . || true; elif command -v flake8 >/dev/null 2>&1; then flake8 . || true; fi; fi; echo "lint complete"'
